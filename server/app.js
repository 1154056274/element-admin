...
//添加电影信息
app.post('/movie/addmovie',(req,res)=>{
 console.log(req.body)
...
app.get('/movie/list', (req, res) => {
  Movie.find().populate('p').exec(function(err, data) {
...
// 删除电影
app.delete('/moviedel/:id', (req, res) => {
  console.log(req.params.id)
  Movie.findByIdAndDelete(req.params.id).populate('p').exec(function(err, data) {
...
//电影分页
app.get('/movielimit', async (req, res) => {
  const page = req.query.page || 1
  const pagesize = req.query.pagesize || 3
  const start = (Number(page)-1) * Number(pagesize)   // 从哪个地方开始查起
  const nums = Number(pagesize)   // 查几条
...
app.get("/getMovieDetail",(req,res)=>{
  var id =  req.query.id
  Movie.findById(id).populate('p').exec(function(err,data){
      res.json({
        code:20000,
        list:data
      })
...
Movie.findByIdAndUpdate(req.query.id,req.body).populate('p').exec(function(err,data) {
    console.log(data)
      res.json({
        code: 20000,
        msg: '数据修改成功'
